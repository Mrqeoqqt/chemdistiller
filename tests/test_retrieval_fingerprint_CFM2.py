...
if __name__=='__main__':
    ...
    test_output_folder='e:/Imperial/TestDB2/HDF5_CFM_large_bestonly';
    ...
    for precision in precisions:                 
        ...
        with PeakAnnotator(dbfiles, fingerprint_model_path, nproc) as peak_annotator:
            ...
            logit('CFM_Recall, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Recall'], 2)], batch, 'CFM_Recall_Multi', use_elements = False, use_formula = False);
            
            logit('CFM_WRecall, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WRecall'], 1)], batch, 'CFM_WRecall_SQRT', use_elements = False, use_formula = False);

            logit('CFM_Recall, Multi, elements');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Recall'], 2)], batch, 'CFM_Recall_Multi_element', use_elements = True, use_formula = False);
            
            logit('CFM_WRecall, SQRT, elements');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WRecall'], 1)], batch, 'CFM_WRecall_SQRT_element', use_elements = True, use_formula = False);


            logit('CFM_Recall,Multi, formula');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Recall'], 2)], batch, 'CFM_Recall_Multi_formula', use_elements = False, use_formula = True);
            
            logit('CFM_WRecall, SQRT,formula');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WRecall'], 1)], batch, 'CFM_WRecall_SQRT_formula', use_elements = False, use_formula = True);


                        
            '''
            logit('CFM_Recall, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Recall'], 1)], batch, 'CFM_Recall_SQRT', use_elements = False, use_formula = False);
            
            logit('CFM_Jaccard, Multi');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Jaccard'], 2)], batch, 'CFM_Jaccard_Multi', use_elements = False, use_formula = False);
            ...
            logit('CFM2_DotProduct, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_DotProduct'], 0)], batch, 'CFM2_DotProduct_Merged', use_elements = True);
            
            logit('CFM2_Jaccard, Merged');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Jaccard'], 0)], batch, 'CFM2_Jaccard_Merged', use_elements = True);
            
            logit('CFM2_WPrecision, Merged');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WPrecision'], 0)], batch, 'CFM2_WPrecision_Merged' , use_elements = True);
            
            logit('CFM2_WRecall, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WRecall'], 0)], batch, 'CFM2_WRecall_Merged', use_elements = True);

            logit('CFM2_Precision, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Precision'], 0)], batch, 'CFM2_Precision_Merged', use_elements = True);

            logit('CFM2_Recall, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Recall'], 0)], batch, 'CFM2_Recall_Merged', use_elements = True);
                   
                   

            logit('CFM2_DotProduct, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_DotProduct'], 1)], batch, 'CFM2_DotProduct_SQRT', use_elements = True);
            
            logit('CFM2_Jaccard, SQRT');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Jaccard'], 1)], batch, 'CFM2_Jaccard_SQRT', use_elements = True);
            
            logit('CFM2_WPrecision, SQRT');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WPrecision'], 1)], batch, 'CFM2_WPrecision_SQRT', use_elements = True);
            
            logit('CFM2_WRecall, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WRecall'], 1)], batch, 'CFM2_WRecall_SQRT', use_elements = True);

            logit('CFM2_Precision, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Precision'], 1)], batch, 'CFM2_Precision_SQRT', use_elements = True);

            logit('CFM2_Recall, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Recall'], 1)], batch, 'CFM2_Recall_SQRT', use_elements = True);
            
            
            
            logit('CFM2_DotProduct, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_DotProduct'], 2)], batch, 'CFM2_DotProduct_Multi', use_elements = True);
            
            logit('CFM2_Jaccard, Multi');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Jaccard'], 2)], batch, 'CFM2_Jaccard_Multi', use_elements = True);
            
            logit('CFM2_WPrecision, Multi');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WPrecision'], 2)], batch, 'CFM2_WPrecision_Multi', use_elements = True);
            
            logit('CFM2_WRecall, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_WRecall'], 2)], batch, 'CFM2_WRecall_Multi', use_elements = True);

            logit('CFM2_Precision, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Precision'], 2)], batch, 'CFM2_Precision_Multi', use_elements = True);

            logit('CFM2_Recall, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer2'],[(subprecision, 0, ['CFM2_Recall'], 2)], batch, 'CFM2_Recall_Multi', use_elements = True);
                   




            logit('CFM_DotProduct, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_DotProduct'], 0)], batch, 'CFM_DotProduct_Merged', use_elements = True);
            
            logit('CFM_Jaccard, Merged');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Jaccard'], 0)], batch, 'CFM_Jaccard_Merged', use_elements = True);
            
            logit('CFM_WPrecision, Merged');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WPrecision'], 0)], batch, 'CFM_WPrecision_Merged' , use_elements = True);
            
            logit('CFM_WRecall, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WRecall'], 0)], batch, 'CFM_WRecall_Merged', use_elements = True);

            logit('CFM_Precision, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Precision'], 0)], batch, 'CFM_Precision_Merged', use_elements = True);

            logit('CFM_Recall, Merged');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Recall'], 0)], batch, 'CFM_Recall_Merged', use_elements = True);
                   
                   

            logit('CFM_DotProduct, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_DotProduct'], 1)], batch, 'CFM_DotProduct_SQRT', use_elements = True);
            
            logit('CFM_Jaccard, SQRT');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Jaccard'], 1)], batch, 'CFM_Jaccard_SQRT', use_elements = True);
            
            logit('CFM_WPrecision, SQRT');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WPrecision'], 1)], batch, 'CFM_WPrecision_SQRT', use_elements = True);
            

            logit('CFM_Precision, SQRT');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Precision'], 1)], batch, 'CFM_Precision_SQRT', use_elements = True);

            
            
            
            logit('CFM_DotProduct, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_DotProduct'], 2)], batch, 'CFM_DotProduct_Multi', use_elements = True);
            
            logit('CFM_Jaccard, Multi');   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Jaccard'], 2)], batch, 'CFM_Jaccard_Multi', use_elements = True);
            
            logit('CFM_WPrecision, Multi');                   
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WPrecision'], 2)], batch, 'CFM_WPrecision_Multi', use_elements = True);
            
            logit('CFM_WRecall, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_WRecall'], 2)], batch, 'CFM_WRecall_Multi', use_elements = True);

            logit('CFM_Precision, Multi');
            for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['CFMScorer'],[(subprecision, 0, ['CFM_Precision'], 2)], batch, 'CFM_Precision_Multi', use_elements = True);

            
            '''       

            '''for subprecision in [0.005]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['FragPrintScorer'],[],batch);
                   #testrun(['FingerPrintScorer'],[[]],batch);

               batch=-1;
               for subprecision in [20]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['FragPrintScorer'],[([0,-1,1], subprecision, 0, 1)],batch);
                   #testrun(['FingerPrintScorer'],[[]],batch);

               for subprecision in [0.005]:
                   logit('batch %s, Fragprint precision %s'%(batch,subprecision));
                   logtime();
                   testrun(['FragPrintScorer'],[([0,-1,1], subprecision, 1, 1)],batch);
                   #testrun(['FingerPrintScorer'],[[]],batch);
            ''' 
