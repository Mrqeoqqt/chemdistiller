...
if __name__=='__main__':
    ...
    def testrun(scorers_list=[], scorers_settings=[], batchindex=-1, header='', use_metalikeness=False, use_formula=False, use_elements = False):
...
if __name__=='__main__':
    ...
    test_spectral_database_path='e:/Imperial/TestDB/NIST14_HMDB_MassBank_5fold_CV_FPT3_withMetaLikeness';
    ...
    test_output_folder='e:/Imperial/TestDB/Redone_HDF5_FingerFrag20ppm_metaonly_andformula_or_element_composition_largedb2_redone2';
    ...
    for precision in precisions:                 
        ...
        for i in reversed(range(len(specmanager.ms_spectra))):
            if int(specmanager.ms_spectra[i].parameters['metalikeness'])==0:
                del specmanager.ms_spectra[i];
        logit('Removed non-meta-like spectra. Total number left: %s'%len(specmanager.ms_spectra));        
        ...
        with PeakAnnotator(dbfiles, fingerprint_model_path, nproc) as peak_annotator:
            ...
            logit('FragPrint 20 ppm, FingerPrint, metabolite likeness 2 class, use formula');
            
            testrun(['FragPrintScorer','FingerPrintScorer'],[([0,-1,1], 20, 0, 1),[]], batch, header = 'Meta+Frag+Finger+formula', use_formula = True, use_metalikeness=True);
            
            
            logit('FragPrint 20 ppm, FingerPrint, metabolite likeness 2 class, use elements');
            
            testrun(['FragPrintScorer','FingerPrintScorer'],[([0,-1,1], 20, 0, 1),[]], batch, header = 'Meta+Frag+Finger+elements', use_elements = True, use_metalikeness=True);
    

            logit('FragPrint 20 ppm, FingerPrint, metabolite likeness 2 class, baseline');
            
            testrun(['FragPrintScorer','FingerPrintScorer'],[([0,-1,1], 20, 0, 1),[]], batch, header = 'Meta+Frag+Finger', use_metalikeness=True);            
            
            
            
            logit('FragPrint 20 ppm, metabolite likeness 2 class, use formula');
            
            testrun(['FragPrintScorer'],[([0,-1,1], 20, 0, 1)], batch, header = 'Meta+Frag+formula', use_formula = True, use_metalikeness=True);
            
            
            logit('FragPrint 20 ppm, metabolite likeness 2 class, use elements');
            
            testrun(['FragPrintScorer'],[([0,-1,1], 20, 0, 1)], batch, header = 'Meta+Frag+elements', use_elements = True, use_metalikeness=True);
    

            logit('FragPrint 20 ppm, metabolite likeness 2 class, baseline');
            
            testrun(['FragPrintScorer'],[([0,-1,1], 20, 0, 1)], batch, header = 'Meta+Frag', use_metalikeness=True);            
            
            
            logit('FingerPrint, metabolite likeness 2 class, use formula');
            
            testrun(['FingerPrintScorer'],[[]], batch, header = 'Meta+Finger+formula', use_formula = True, use_metalikeness=True);
            
            
            logit('FingerPrint, metabolite likeness 2 class, use elements');
            
            testrun(['FingerPrintScorer'],[[]], batch, header = 'Meta+Finger+elements', use_elements = True, use_metalikeness=True);
    

            logit('FingerPrint, metabolite likeness 2 class, baseline');
            
            testrun(['FingerPrintScorer'],[[]], batch, header = 'Meta+Finger', use_metalikeness=True);            
            
            
            
            '''
            logit('FragPrint 20 ppm, FingerPrint, metabolite likeness 2 class raw');
            
            testrun(['FragPrintScorer','FingerPrintScorer'],[([0,-1,1], 20, 0, 1),[]], batch, use_metalikeness=True, use_formula=False);
            '''
