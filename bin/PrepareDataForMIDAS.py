...
infile='e:/Imperial/TestDB/testmassranges_0.005_batch0_new.txt';
...
for i in reversed(range(len(onlyfiles))):
    fn=onlyfiles[i];
    print(fn)
    if not (fn.startswith('TestDB') or fn.startswith('MassBank') or fn.startswith('PubChem') or fn.startswith('ChEBI') or fn.startswith('HMDB')):
        del onlyfiles[i];
print(onlyfiles)
...
for dbfile in onlyfiles:
    if dbfile.lower().endswith('.h5'):
        ...
        with h5py.File(os.path.join(dbpath,dbfile)) as hdf5_container:
            ...
            with open('g:/MIDAS/db_candidates_%s.txt'%os.path.splitext(dbfile)[0],'w') as fout:
                massranges=[];
                ...
                for i in range(total_count):
                    ...
                    if mz>=massrange[0] and mz<=massrange[1]:
                        ...
                        idname = os.path.splitext(dbfile)[0]+'_%s'%i;
